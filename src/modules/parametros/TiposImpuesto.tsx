...
import type { TipoImpuesto } from '../../types';

type FormData = Omit<TipoImpuesto, 'id'>;
...
export default function TiposImpuestoPage() {
  ...
  const [editing, setEditing] = useState<TipoImpuesto | null>(null);
  const [form, setForm] = useState<FormData>(empty);
  const [deleteId, setDeleteId] = useState<string | null>(null);

  const openNew = () => { setEditing(null); setForm(empty); setModalOpen(true); };
  const openEdit = (t: TipoImpuesto) => { setEditing(t); setForm({ nombre: t.nombre, porcentaje: t.porcentaje, activo: t.activo }); setModalOpen(true); };

  const handleSave = () => {
    if (!form.nombre.trim()) { toast.error('El nombre es requerido'); return; }
    if (editing) {
      dispatch({ type: 'UPDATE_TIPO_IMPUESTO', payload: { ...editing, ...form } });
      toast.success('Tipo de impuesto actualizado');
    } else {
      dispatch({ type: 'ADD_TIPO_IMPUESTO', payload: { id: uuid(), ...form } });
      toast.success('Tipo de impuesto creado');
    }
    setModalOpen(false);
  };

  const columns: Column<TipoImpuesto>[] = [
  ...
  return (
    <div>
      <PageHeader title="Tipos de Impuesto" subtitle="Configure los tipos de impuesto aplicables" action={<Button icon={<Plus size={16} />} onClick={openNew}>Nuevo</Button>} />
      <Table columns={columns} data={state.tiposImpuesto} />

      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar Tipo de Impuesto' : 'Nuevo Tipo de Impuesto'} size="sm"
        footer={<><Button variant="secondary" onClick={() => setModalOpen(false)}>Cancelar</Button><Button onClick={handleSave}>Guardar</Button></>}
      >
        <div className="space-y-4">
          <Input as="input" label="Nombre" value={form.nombre} onChange={e => setForm(p => ({ ...p, nombre: e.target.value }))} placeholder="Ej: IVA 15%" />
      ...
      <Modal open={!!deleteId} onClose={() => setDeleteId(null)} title="Confirmar Eliminación"
        footer={<><Button variant="secondary" onClick={() => setDeleteId(null)}>Cancelar</Button><Button variant="danger" onClick={() => { if (deleteId) { dispatch({ type: 'DELETE_TIPO_IMPUESTO', payload: deleteId }); toast.success('Eliminado'); setDeleteId(null); } }}>Eliminar</Button></>}
      >
        <p className="text-gray-600 dark:text-slate-300">¿Desea eliminar este tipo de impuesto?</p>
      </Modal>
    </div>
  );
}
